...
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../plugins/pipeline-core/scripts/platform" && pwd)"
  ...
  pr)
    case "$2" in
      create)
        ...
        ;;
      view)
        # wait_for_mergeable polls: gh pr view N --json mergeable --jq .mergeable
        if [[ "$*" == *"--json mergeable"* ]]; then
          echo "${MOCK_GH_PR_MERGEABLE:-MERGEABLE}"
        else
