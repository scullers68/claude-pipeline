...
> **Graceful fallback:** If `${CLAUDE_PLUGIN_ROOT}/skills/test-discovery/SKILL.md` does not exist, skip this sub-step and note "test-discovery skill unavailable" in the research findings.
...
- [ ] `[bash-script-craftsman]` **(S)** Description of script task — `${CLAUDE_PLUGIN_ROOT}/scripts/deploy.sh:L30-65`
...
**Agent values** — use agents defined in `${CLAUDE_PLUGIN_ROOT}/agents/`:
