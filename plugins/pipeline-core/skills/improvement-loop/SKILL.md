...
1. **Save the issue body as a fixture file**
   ...
   cp <issue-body-source> \
     ${CLAUDE_PLUGIN_ROOT}/scripts/implement-issue-test/fixtures/triage/<issue-id>.md
   ```
...
3. **Add a manifest entry to `${CLAUDE_PLUGIN_ROOT}/skills/triage-classify/golden.manifest.txt`**
...
4. **Verify the new fixture passes**
   ...
   ```bash
   ${CLAUDE_PLUGIN_ROOT}/scripts/skill-golden.sh triage-classify <issue-id>
   ```
...
5. **Commit both files**
   ...
   ```bash
   git add ${CLAUDE_PLUGIN_ROOT}/skills/triage-classify/golden.manifest.txt \
           ${CLAUDE_PLUGIN_ROOT}/scripts/implement-issue-test/fixtures/triage/<issue-id>.md
