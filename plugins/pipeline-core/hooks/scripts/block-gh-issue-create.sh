...
#   `${CLAUDE_PLUGIN_ROOT}/scripts/create-followup-issue.sh ...`, that is the command the
...
if pattern.search(cmd):
    sys.stderr.write(
        ...
        "${CLAUDE_PLUGIN_ROOT:-<pipeline-plugin>}/scripts/create-followup-issue.sh (follow-ups) or "
        "${CLAUDE_PLUGIN_ROOT:-<pipeline-plugin>}/scripts/platform/create-issue.sh — they run assert_issue_valid "
        "fail-closed so every issue has parseable tasks + acceptance criteria. "
        "See the process-pr skill, Step 4g.\n")
    sys.exit(2)
sys.exit(0)
'
