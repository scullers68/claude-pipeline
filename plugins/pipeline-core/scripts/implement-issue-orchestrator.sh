...
# six criteria and scripts/triage-validate.sh for prompt-quality tests.
#
# Arguments:
...
#
# Returns 0 if <name>.md exists in any agent search dir. Search order:
# PIPELINE_AGENTS_DIRS (colon-separated) when set, else the core plugin's
# agents/ plus the consumer project's .claude/agents.
_agent_defined() {
	local name="$1" dirs d
	dirs="${PIPELINE_AGENTS_DIRS:-${SCRIPT_DIR}/../agents${CLAUDE_PROJECT_DIR:+:${CLAUDE_PROJECT_DIR}/.claude/agents}}"
	IFS=':' read -ra _agent_dirs <<< "$dirs"
	for d in "${_agent_dirs[@]}"; do
		[[ -n "$d" && -f "$d/$name.md" ]] && return 0
	done
	return 1
}
    ...
    while IFS= read -r file; do
        ...
        case "$file" in
            *.ts|*.tsx|*.js|*.jsx|*.mjs|*.cjs) has_ts=true ;;
            # Pipeline shell code (plugins/*/scripts/, tests/) has bats tests —
            # route to bash. Legacy consumer copies keep the .claude/scripts path.
            plugins/*/scripts/*.sh) has_bash=true ;;
            tests/*.sh) has_bash=true ;;
...
_build_bash_test_command() {
	...
	if [[ -f "$loop_dir/tests/implement-issue-test/run-tests.sh" ]]; then
		bash_test_command="bash tests/implement-issue-test/run-tests.sh"
	elif [[ -f "$loop_dir/.claude/scripts/implement-issue-test/run-tests.sh" ]]; then
...
run_test_loop() {
    ...
    while [[ "$loop_complete" != "true" ]]; do
        ...
        # Build BATS section.
        # bash scope (pipeline shell changes): BLOCKING — failures fail the stage.
    ...
    if [[ -n "$RESUME_MODE" ]] && is_stage_completed "validate_plan"; then
        ...
        for ((i=0; i<task_count; i++)); do
            ...
            if [[ "$check_agent" != "default" ]] \
                && ! _agent_defined "$check_agent"; then
                log "WARNING: Task $((i+1)) uses agent '$check_agent'" \
                    "which has no definition in the plugin agents/ directory"
            fi
        done
...
\`\`\`
        ...
        # ---------------------------------------------------------------------
        local bats_runner="tests/implement-issue-test/run-tests.sh"
        [[ -f "$bats_runner" ]] || bats_runner=".claude/scripts/implement-issue-test/run-tests.sh"
